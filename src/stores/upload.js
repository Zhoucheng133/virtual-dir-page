...
export default defineStore('upload', ()=>{
  ...
  const uploadURL=()=>{
    return `${stores().baseURL}/api/upload?username=${stores().userData.username}&password=${CryptoJS.SHA256(stores().userData.password).toString()}&path=${encodeURIComponent(JSON.stringify(stores().path.slice(1)))}&isDir=false`;
  }

  const dirUploadURL=(path)=>{
    return `${stores().baseURL}/api/upload?username=${stores().userData.username}&password=${CryptoJS.SHA256(stores().userData.password).toString()}&path=${encodeURIComponent(JSON.stringify([...stores().path.slice(1), path]))}&isDir=true`;
  }
