...
export default defineStore('index', ()=>{
  ...
  const fileDownload=(item)=>{
    const url=window.location.href=`${baseURL}/api/download?username=${userData.value.username}&password=${CryptoJS.SHA256(userData.value.password).toString()}&path=${encodeURIComponent(JSON.stringify([...path.value, item].slice(1)))}`;
    window.location.href=url;
  }

  const archiveDownload=(items)=>{
    const url=`${baseURL}/api/multidownload?username=${userData.value.username}&password=${CryptoJS.SHA256(userData.value.password).toString()}&path=${encodeURIComponent(JSON.stringify([...path.value].slice(1)))}&files=${JSON.stringify(items)}`
    window.location.href=url;
  }
